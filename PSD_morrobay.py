...
dsamp_factor, dx_correct,channel_bin, channel_interval, correction_factor, amp_type='strain_rate', on_macos=False):
    ...
    for i, chan in tqdm(enumerate(ch_list)):
        ...
        H,xm,ym = ppsd(trs,new_fs,1/500,new_fs)
    ...
    plt.tight_layout()  
    plt.savefig(out_dir + 'PSD-'+str(file_list[0])+str(file_list[-1])+'.png',dpi=300)


if __name__ == '__main__':

    for test_dir in ['MorroPhase_sweep40us_gauge10m/20241106/dphi/',
                    'MorroPhase_sweep_gauge40m/20241105/dphi/']:
        since = time.time()
        ppsd_on_fly(data_dir = '/data/data4/morro_bay_test/'+test_dir, 
                    out_dir = '/data/data4/morro_bay_test/',
